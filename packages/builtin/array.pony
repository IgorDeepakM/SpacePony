class Array[A : AnyNoCheck] is Seq[A]
  ...
  =>
    ...
    """
    _ptr.pointer_at_index(from_offset)._copy_to(ptr.pointer_at_index(to_offset), copy_len)

  fun cpointer(offset: USize = 0): Pointer[A] tag =>
    """
    Return the underlying C-style pointer.
    """
    _ptr.pointer_at_index(offset)
  ...
  fun ref delete(i: USize): A^ ? =>
    ...
    if i < _size then
      _size = _size - 1
      _ptr.pointer_at_index(i)._delete(1, _size - i)
    else
      error
    end
  ...
  fun ref trim_in_place(from: USize = 0, to: USize = -1) =>
    ...
    if _alloc == 0 then
      ...
      _ptr = _ptr.pointer_at_index(offset)
    end
  ...
  fun val trim(from: USize = 0, to: USize = -1): Array[A] val =>
    ...
    recover
      ...
      if size' > 0 then
        from_cpointer(_ptr.pointer_at_index(offset)._unsafe(), size', alloc)
      else
        create()
      end
    end
  ...
  =>
    ...
    """
    reserve(dst_idx + len)
    src._ptr.pointer_at_index(src_idx)._copy_to(_ptr.convert[U8]().pointer_at_index(dst_idx), len)
  ...
  =>
    ...
    if (src_idx < _size) and (dst_idx <= dst._size) then
      ...
      if count > 0 then
        ...
        _ptr.pointer_at_index(src_idx)._copy_to(dst._ptr.pointer_at_index(dst_idx), count)
  ...
  fun ref remove(i: USize, n: USize) =>
    ...
    if i < _size then
      ...
      _size = _size - count
      _ptr.pointer_at_index(i)._delete(count, _size - i)
    end
