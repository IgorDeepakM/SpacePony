...
```
  ...
  fun at(s: String box, offset: ISize = 0): Bool =>
    ...
    if (i + s.size()) <= _size then
      @memcmp(_ptr.pointer_at_index(i), s._ptr, s._size) == 0
    else
      false
    end
  ...
  fun ref delete(offset: ISize, len: USize = 1) =>
    ...
    if i < _size then
      ...
      _size = _size - n
      _ptr.pointer_at_index(i)._delete(n, _size - i)
      _set(_size, 0)
    end
  ...
  fun substring(from: ISize, to: ISize = ISize.max_value()): String iso^ =>
    ...
    if (start < _size) and (start < finish) then
      ...
      let str = recover String(len) end
      _ptr.pointer_at_index(start)._copy_to(str._ptr._unsafe(), len)
      str._size = len
      str._set(len, 0)
      str
    else
  ...
  fun ref pop(): U8 ? =>
    ...
    if _size > 0 then
      _size = _size - 1
      _ptr.pointer_at_index(_size)._delete(1, 0)
    else
      error
    end
  ...
  fun ref insert_in_place(offset: ISize, that: String box) =>
    ...
    that._ptr._copy_to(_ptr.pointer_at_index(index), that._size)
