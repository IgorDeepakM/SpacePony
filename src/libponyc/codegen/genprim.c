...
static void pointer_pointer_at_index(compile_t* c, void* data, token_id cap)
{
  reach_type_t* t = ((reach_type_t**)data)[0];
...
static void c_fixed_sized_array_cpointer(compile_t* c, reach_type_t* t, reach_method_t* m, void* gen_data)
{
  ...
  LLVMValueRef ptr = LLVMGetParam(c_m->func, 0);
  LLVMValueRef array_ptr = LLVMBuildStructGEP2(c->builder, c_t->structure, ptr, 0, "");
  genfun_build_ret(c, array_ptr);
  codegen_finishfun(c);
}
  ...
  LLVMValueRef ctx, LLVMValueRef array_ptr)
{
  ...
  // Build a GEP to the element of the fixed sized array
  ...
  LLVMValueRef elem = LLVMBuildLoad2(c->builder, c_t_elem->mem_type, elem_ptr,
    "");
  elem = gen_assign_cast(c, c_t_elem->use_type, elem, elem_type);
...
void genprim_c_fixed_sized_array_serialise(compile_t* c, reach_type_t* t)
{
  ...
  LLVMValueRef array_ptr_dest = field_loc(c, offset_addr, c_t->structure, 0);

  if ((t_elem->underlying == TK_PRIMITIVE) && (c_t_elem->primitive != NULL))
  {
    LLVMValueRef array_ptr_src = field_loc(c, object, c_t->structure, 0);
    gencall_memcpy(c, array_ptr_dest, array_ptr_src, size);
  }
  else
  {
    LLVMBasicBlockRef entry_block = LLVMGetInsertBlock(c->builder);
    ...
    LLVMValueRef elem_ptr = LLVMBuildInBoundsGEP2(c->builder, c_t_elem->mem_type, object, &phi, 1,
      "");

    LLVMValueRef l_size = LLVMConstInt(c->intptr, abisize, false);
    LLVMValueRef elem_offset = LLVMBuildMul(c->builder, phi, l_size, "");
    LLVMValueRef elem_loc = LLVMBuildInBoundsGEP2(c->builder, c->i8, array_ptr_dest,
      &elem_offset, 1, "");
    ...
    LLVMValueRef inc = LLVMBuildAdd(c->builder, phi, one, "");
    body_block = LLVMGetInsertBlock(c->builder);
    LLVMAddIncoming(phi, &inc, &body_elem_block, 1);
    LLVMBuildBr(c->builder, cond_elem_block);
...
static void type_trait_gen_bool(compile_t* c, reach_type_t* t, reach_method_t* m, bool ret)
{
  ...
  genfun_build_ret(c, ret_val);
  codegen_finishfun(c);
}

static void type_trait_is_struct(compile_t* c, reach_type_t* t, reach_method_t* m, void* gen_data)
{
  (void)gen_data;
  m->intrinsic = true;
  ast_t* typearg = ast_child(m->typeargs);

  type_trait_gen_bool(c, t, m, is_entity(typearg, TK_STRUCT));
}

static void type_trait_is_class(compile_t* c, reach_type_t* t, reach_method_t* m, void* gen_data)
{
  (void)gen_data;
  m->intrinsic = true;
  ast_t* typearg = ast_child(m->typeargs);

  type_trait_gen_bool(c, t, m, is_entity(typearg, TK_CLASS));
}

static void type_trait_is_primitive(compile_t* c, reach_type_t* t, reach_method_t* m, void* gen_data)
{
  (void)gen_data;
  m->intrinsic = true;
  ast_t* typearg = ast_child(m->typeargs);

  type_trait_gen_bool(c, t, m, is_entity(typearg, TK_PRIMITIVE));
}

static void type_trait_is_subtype(compile_t* c, reach_type_t* t, reach_method_t* m, void* gen_data)
{
  (void)gen_data;
  m->intrinsic = true;
  ast_t* sub = ast_child(m->typeargs);
  ast_t* super = ast_childidx(m->typeargs, 1);

  type_trait_gen_bool(c, t, m, is_subtype(sub, super, NULL, c->opt));
}

static void type_trait_is_subtype_constraint(compile_t* c, reach_type_t* t, reach_method_t* m, void* gen_data)
{
  (void)gen_data;
  m->intrinsic = true;
  ast_t* sub = ast_child(m->typeargs);
  ...
  type_trait_gen_bool(c, t, m, is_subtype_constraint(sub, super, NULL, c->opt));
}

static void type_trait_is_subtype_ignore_cap(compile_t* c, reach_type_t* t, reach_method_t* m, void* gen_data)
{
  (void)gen_data;
  m->intrinsic = true;
  ast_t* sub = ast_child(m->typeargs);
  ...
  type_trait_gen_bool(c, t, m, is_subtype_ignore_cap(sub, super, NULL, c->opt));
}

static void type_trait_is_eqtype(compile_t* c, reach_type_t* t, reach_method_t* m, void* gen_data)
{
  (void)gen_data;
  m->intrinsic = true;
  ast_t* sub = ast_child(m->typeargs);
  ...
  type_trait_gen_bool(c, t, m, is_eqtype(sub, super, NULL, c->opt));
}

static void type_trait_is_bool(compile_t* c, reach_type_t* t, reach_method_t* m, void* gen_data)
{
  (void)gen_data;
  m->intrinsic = true;
  ast_t* typearg = ast_child(m->typeargs);

  type_trait_gen_bool(c, t, m, is_bool(typearg));
}

static void type_trait_is_float(compile_t* c, reach_type_t* t, reach_method_t* m, void* gen_data)
{
  (void)gen_data;
  m->intrinsic = true;
  ast_t* typearg = ast_child(m->typeargs);

  type_trait_gen_bool(c, t, m, is_float(typearg));
}

static void type_trait_is_integer(compile_t* c, reach_type_t* t, reach_method_t* m, void* gen_data)
{
  (void)gen_data;
  m->intrinsic = true;
  ast_t* typearg = ast_child(m->typeargs);

  type_trait_gen_bool(c, t, m, is_integer(typearg));
}

static void type_trait_is_machine_word(compile_t* c, reach_type_t* t, reach_method_t* m, void* gen_data)
{
  (void)gen_data;
  m->intrinsic = true;
  ast_t* typearg = ast_child(m->typeargs);

  type_trait_gen_bool(c, t, m, is_machine_word(typearg));
}

static void type_trait_is_signed(compile_t* c, reach_type_t* t, reach_method_t* m, void* gen_data)
{
  (void)gen_data;
  m->intrinsic = true;
  ast_t* typearg = ast_child(m->typeargs);

  type_trait_gen_bool(c, t, m, is_signed(typearg));
}

void genprim_type_traits_methods(compile_t* c, reach_type_t* t)
{
  GENERIC_FUNCTION("is_struct", type_trait_is_struct, NULL);
  GENERIC_FUNCTION("is_class", type_trait_is_class, NULL);
  GENERIC_FUNCTION("is_primitive", type_trait_is_primitive, NULL);
  GENERIC_FUNCTION("is_subtype", type_trait_is_subtype, NULL);
  GENERIC_FUNCTION("is_subtype_constraint", type_trait_is_subtype_constraint, NULL);
  GENERIC_FUNCTION("is_subtype_ignore_cap", type_trait_is_subtype_ignore_cap, NULL);
  GENERIC_FUNCTION("is_eqtype", type_trait_is_eqtype, NULL);
  GENERIC_FUNCTION("is_bool", type_trait_is_bool, NULL);
  GENERIC_FUNCTION("is_float", type_trait_is_float, NULL);
  GENERIC_FUNCTION("is_integer", type_trait_is_integer, NULL);
  GENERIC_FUNCTION("is_machine_word", type_trait_is_machine_word, NULL);
  GENERIC_FUNCTION("is_signed", type_trait_is_signed, NULL);
}
