...
static void init_runtime(compile_t* c)
{
  ...
  c->str_CFixedSizedArray = stringtab(c->opt->strtab, "CFixedSizedArray");
  c->str_Atomic = stringtab(c->opt->strtab, "Atomic");
...
{
  LLVMValueRef str_val = LLVMConstStringInContext2(c->context, str, len, false);
