...
LLVMValueRef gen_call(compile_t* c, ast_t* ast)
{
  ...
  if(is_message)
  {
    ...
    while(arg != NULL)
    {
      ...
      {
        args[i] = load_lowered_param_value_from_ptr(c, args[i], params[i], pt);
      }
      else
      {
  ...
  bool intrinsic)
{
  ...
  if(t->underlying == TK_TUPLETYPE)
  {
    ...
    {
      // Tuples are always return by value
      ffi_decl->return_by_value = true;
    }
  }
  ...
  ast_t* args, ast_t* ret_ast, bool intrinsic)
{
  ...
  bool return_by_value = ffi_decl->return_by_value;
  ...
  ffi_decl->num_params = param_count;
  ...
  if(param_count != 0)
  {
    ...
    size_t ffi_param_size = param_count * sizeof(ffi_param_t);
    ffi_decl->params = (ffi_param_t*)ponyint_pool_alloc_size(ffi_param_size);
    for(int i = 0; i < param_count; i++)
    {
      ffi_decl->params[i].reach_type = NULL;
      ffi_decl->params[i].pass_by_value = false;
    }
...
LLVMValueRef gen_ffi(compile_t* c, ast_t* ast)
{
  ...
  if(ffi_decl != NULL)
  {
    ...
    return_value_lowered = ffi_decl->return_value_lowered;
    func = ffi_decl->func;
  }
  ...
  if(return_by_value)
  {
    if(t->underlying == TK_TUPLETYPE)
    {
      assign_side = LLVMBuildAlloca(c->builder, c_t->use_type, "");
    }
    else
    {
  ...
  for(int i = arg_start; i < llvm_arg_count; i++)
  {
    ...
    if(!vararg)
    {
      ...
      {
        f_args[i] = cast_ffi_arg(c, ffi_decl, ast, f_args[i], f_params[i], "parameters");
      }
    }
  ...
  result = cast_ffi_arg(c, ffi_decl, ast, result, c_t->use_type, "return values");
...
void gencall_lifetime_start(compile_t* c, LLVMValueRef ptr)
{
  ...
  LLVMValueRef args[1];
  args[0] = ptr;
  LLVMBuildCall2(c->builder, func_type, func, args, 1, "");
}
